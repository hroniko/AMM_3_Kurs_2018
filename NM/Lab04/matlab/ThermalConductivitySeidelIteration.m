function [X, T, P, P1] = ThermalConductivitySeidelIteration(x1, x2, t1, t2, Nx, Nt, cicles)
% Функция для решения начальной краевой задачи для уравнения
% теплопроводности в прямоугольнике с помощью итерационного метода Зейделя
...
for c = 1 : cicles
    ...
    for ti = 2 : Nt % по каждой строке
        for xj = 2 : Nx % по каждому столбцу
            P(ti, xj) = 1/4 * ( P(ti-1, xj) + P(ti+1, xj) + P(ti, xj-1) + P(ti, xj+1) ) - 0.5/( 1/(h^2) + 1/(t^2) ) * F(xj, ti);        
        end    
    end
end
