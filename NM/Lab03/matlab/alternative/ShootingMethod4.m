function [stPz, k, enPz_rs] = ShootingMethod4(a, b, N, stPy, enPz)
...
enPz_rs = 0.5; % приближенное значение решения

k = 0; % счетчик шагов
while abs(stPz_r - stPz_l) > 0.01 % пока шаг превышет погрешность, считаем дальше    
    ...
    % Считаем погрешности
    dl = deltaByDoubleSolve(a, b, N, stPy, stPz_l);
    dr = deltaByDoubleSolve(a, b, N, stPy, stPz_r);
    d0 = deltaByDoubleSolve(a, b, N, stPy, stPz);
...
%     end   
    ...
    if abs(dl-d0) < abs(dr-d0) % Zl(N+1) < enPz & enPz < Zo(N+1)
        stPz_r = stPz;        
        enPz_rs = Zo(N+1);        
        continue;
    end    
    if abs(dl-d0) > abs(dr-d0) % Zo(N+1) < enPz & enPz < Zr(N+1)
        stPz_l = stPz;        
        enPz_rs = Zo(N+1);
        continue;
    end    
